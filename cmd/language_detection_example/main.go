...
func main() {
	...
	// Пример Markdown с разными блоками кода
	markdown := `# Примеры кода на разных языках
...
`
	...
	if err != nil {
		log.Fatalf("Ошибка при анализе Markdown: %v", err)
	}

	fmt.Printf("Найдено %d блоков кода:\n\n", len(blocks))
	...
	for i, block := range blocks {
		...
		fmt.Printf("  Явно указан: %t\n", block.HasExplicit)
		fmt.Printf("  Содержимое (первые 100 символов): %s\n",
			truncateString(block.Content, 100))
		fmt.Println()
	}
}
